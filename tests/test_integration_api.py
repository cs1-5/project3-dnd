...
class TestMonsterCreationFromAPI:
    ...
    def test_create_goblin_from_api(self):
        ...
        # Verify derived attributes computed correctly
        # Goblin has STR 8 → modifier -1, proficiency +2 → attack bonus +1
        assert goblin.attack_bonus == 1
        
        # Verify damage dice notation
        assert "d6" in goblin.damage_dice  # Goblins use d6 for damage
        assert goblin.damage_dice_parsed[1] == 6  # Die size should be 6
    ...
    def test_create_ancient_dragon_from_api(self):
        ...
        # Strong attack bonus (STR mod + proficiency)
        assert dragon.attack_bonus > 10
    ...
    def test_multiple_monsters_can_be_created(self):
        ...
        for monster in monsters:
            assert monster.hp > 0
            assert monster.ac > 0
            # Attack bonus can be 0, negative, or positive - just check it's an integer
            assert isinstance(monster.attack_bonus, int)
